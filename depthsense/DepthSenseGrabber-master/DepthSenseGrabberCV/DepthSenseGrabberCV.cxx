...
const int nPixelsDepthFloatRaw = widthDepth*heightDepth*3;
...
uint8_t pixelsDepthFloatRaw[nPixelsDepthFloatRaw];
    ...
    if (data.depthMap!=0)// just in case !
        for (int i=0; i<heightDepth; i++)
            for (int j=0; j<widthDepth; j++)
            {
                ...
                depth = data.depthMapFloatingPoint[countDepth]*255;
                if (data.depthMap[countDepth] < noDepthThreshold)
                    pixelsDepthRaw[countDepth] = data.depthMap[countDepth];
                    pixelsDepthFloatRaw[3*countDepth] = depth;
                    pixelsDepthFloatRaw[3*countDepth+1] = depth;
                    pixelsDepthFloatRaw[3*countDepth+2] = depth;
                else
                    pixelsDepthRaw[countDepth] = noDepthDefault;
                    pixelsDepthFloatRaw[3*countDepth] = 0;
                    pixelsDepthFloatRaw[3*countDepth+1] = 0;
                    pixelsDepthFloatRaw[3*countDepth+2] = 0;
                ...
                if (dispColorSyncFlag || (saveColorSyncFlag && exportJPGFlag)) cvSet2D(g_colorSyncImage,i,j,cvScalar(pixelsDepthFloatRaw[3*countDepth+2],pixelsDepthFloatRaw[3*countDepth+1],pixelsDepthFloatRaw[3*countDepth])); //BGR format
                countDepth++;
            }
